...
router.get("/", optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    const { products: productsList, total } = await memoryDb.findProducts({
      search,
      category,
      featured,
      limit,
...
router.get("/:id", optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    const product = await memoryDb.findProductById(id);
    ...
    res.json({
      ...
      data: {
        ...product,
        images: [] // No image support in memory DB for now
      }
    });
  } catch (error) {
...
router.post("/", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const newProduct = await memoryDb.createProduct({
      ...productData,
      // Convert arrays and objects to strings for memory storage
      highlights: productData.highlights ? JSON.stringify(productData.highlights) : "",
      technicalSpecs: productData.technicalSpecs ? JSON.stringify(productData.technicalSpecs) : "",
      createdAt: new Date(),
      updatedAt: new Date()
    } as any);
...
router.put("/:id", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const existingProduct = await memoryDb.findProductById(id);
    if (!existingProduct) {
      return res.status(404).json({
        success: false,
        message: "Produto não encontrado"
    ...
    // Update product
    const updatedProduct = await memoryDb.updateProduct(id, {
      ...productData,
...
router.delete("/:id", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    await memoryDb.updateProduct(id, { 
      isActive: false,
      updatedAt: new Date()
    });
